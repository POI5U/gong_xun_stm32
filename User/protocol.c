...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  uint8_t sum = 0;    // 校验和
  num *= 4;           // 一个参数 4 个字节
  uint8_t data_array[20] = {0};
  uint8_t *dat = (uint8_t*)data;
  static packet_head_t set_packet;
  
  set_packet.head = (int32_t)FRAME_HEADER;     // 包头 0x59485A53
  set_packet.len  = (int8_t)0x0B + num;      // 包长
  set_packet.ch   = (int8_t)ch;              // 设置通道
  set_packet.cmd  = (int32_t)cmd;             // 设置命令
  
  sum = check_sum(0, (uint8_t *)&set_packet, sizeof(set_packet));       // 计算包头校验和
  sum = check_sum(sum, (uint8_t *)data, num);                           // 计算参数校验和
  
  data_array[0] = (set_packet.head) & 0xff;
  data_array[1] = (set_packet.head>>8) & 0xff;
  
  data_array[2] = (set_packet.head>>16) & 0xff;
  data_array[3] = (set_packet.head>>24) & 0xff;
  data_array[4] = set_packet.ch ;
  data_array[5] = 0x0F;
  data_array[6] = 0x00;
  data_array[7] = 0x00;
  data_array[8] = 0x00;
  ...
  HAL_UART_Transmit(&huart1, (uint8_t *)&data_array, 15, 0xFFFFF);    // 发送数据头
