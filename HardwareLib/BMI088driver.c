...
static int16_t gyroz_zero_error = 0;
...
static int16_t BMI088_read_gz_int16(void)
{
	uint8_t buf[2] = { 0 };
	int16_t recv_data;
    BMI088_gyro_read_muli_reg(BMI088_GYRO_CHIP_ID+6, buf, 2);
	
	recv_data = (int16_t)(buf[1] << 8) | buf[0];
	return recv_data;
}

/**
************************************************************************
* @brief:      	void get_gz_error(void)
* @param:		get_times: 读取的次数, 越多越平稳
* @details:    	得到陀螺仪z轴零飘, 并保存备用
************************************************************************
**/
int16_t get_gz_error(uint16_t get_times)
{
	int32_t all_err = 0;
	for(uint16_t i = 0; i < get_times; i++){
		all_err += BMI088_read_gz_int16();
	}
	
	all_err /= get_times;
	gyroz_zero_error = all_err;
	return all_err;
}
...
/**
************************************************************************
* @brief:      	float BMI088_read_gz(void)
* @retval:     	float: 得到的gyro关于z轴的值(度每秒)
* @details:    	读取BMI088传感器陀螺仪
************************************************************************
**/
float BMI088_read_gz(void)
{
	float d_cont = 0.1;
	uint8_t buf[2] = { 0 };
	int16_t recv_data;
	...
	recv_data = (int16_t)(buf[1] << 8) | buf[0];
	recv_data -= gyroz_zero_error;
	
	recv_data = d_cont * recv_data + (1 - d_cont) * last_data;
	last_data = recv_data;

	float data = (float)recv_data * 4000 / 0xFFFF;
...
/**
************************************************************************
* @brief:      	float BMI088_read_temp(void)
* @retval:     	float: 得到的温度
* @details:    	读取BMI088传感器温度
************************************************************************
**/
